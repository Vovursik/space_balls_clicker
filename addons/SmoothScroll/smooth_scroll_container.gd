...
@export var disable_scrollbar: bool = false:
	set(val):
		disable_scrollbar = val
		if is_inside_tree():
			_apply_scrollbar_disable_state()
...
func _apply_scrollbar_disable_state() -> void:
	var v_bar = get_v_scroll_bar()
	var h_bar = get_h_scroll_bar()
	
	if disable_scrollbar:
		# Полностью отключаем скроллбары
		v_bar.hide()
		h_bar.hide()
		v_bar.mouse_filter = Control.MOUSE_FILTER_IGNORE
		h_bar.mouse_filter = Control.MOUSE_FILTER_IGNORE
		
		# Отключаем все сигналы, связанные со скроллбарами
		if v_bar.gui_input.is_connected(_scrollbar_input):
			v_bar.gui_input.disconnect(_scrollbar_input)
		if h_bar.gui_input.is_connected(_scrollbar_input):
			h_bar.gui_input.disconnect(_scrollbar_input)
		if v_bar.mouse_entered.is_connected(_mouse_on_scroll_bar):
			v_bar.mouse_entered.disconnect(_mouse_on_scroll_bar)
		if v_bar.mouse_exited.is_connected(_mouse_on_scroll_bar):
			v_bar.mouse_exited.disconnect(_mouse_on_scroll_bar)
		if h_bar.mouse_entered.is_connected(_mouse_on_scroll_bar):
			h_bar.mouse_entered.disconnect(_mouse_on_scroll_bar)
		if h_bar.mouse_exited.is_connected(_mouse_on_scroll_bar):
			h_bar.mouse_exited.disconnect(_mouse_on_scroll_bar)
		
		# Останавливаем таймер авто-скрытия
		if scrollbar_animator:
			scrollbar_animator.stop_hide_timer()
	else:
		# Возвращаем нормальное поведение
		v_bar.mouse_filter = Control.MOUSE_FILTER_STOP
		h_bar.mouse_filter = Control.MOUSE_FILTER_STOP
		
		# Подключаем сигналы обратно
		if not v_bar.gui_input.is_connected(_scrollbar_input):
			v_bar.gui_input.connect(_scrollbar_input.bind(true))
		if not h_bar.gui_input.is_connected(_scrollbar_input):
			h_bar.gui_input.connect(_scrollbar_input.bind(false))
		if not v_bar.mouse_entered.is_connected(_mouse_on_scroll_bar):
			v_bar.mouse_entered.connect(_mouse_on_scroll_bar.bind(true))
		if not v_bar.mouse_exited.is_connected(_mouse_on_scroll_bar):
			v_bar.mouse_exited.connect(_mouse_on_scroll_bar.bind(false))
		if not h_bar.mouse_entered.is_connected(_mouse_on_scroll_bar):
			h_bar.mouse_entered.connect(_mouse_on_scroll_bar.bind(true))
		if not h_bar.mouse_exited.is_connected(_mouse_on_scroll_bar):
			h_bar.mouse_exited.connect(_mouse_on_scroll_bar.bind(false))
		
		# Восстанавливаем видимость согласно настройкам
		if hide_scrollbar_over_time:
			scrollbar_animator.start_hide_timer()
		else:
			v_bar.show()
