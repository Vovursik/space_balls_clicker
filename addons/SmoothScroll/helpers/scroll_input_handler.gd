...
## Flag to track if focus has been released during current drag
var _focus_released: bool = false
## Flag to track if scrolling actually occurred during drag
var _was_scrolled: bool = false
## Flag to track if drag started (for button click detection)
var _drag_started: bool = false

var scroll_zone: float = 100.0
#endregion
...
## Returns true if scrolling occurred during the current drag gesture.
func was_scrolled_during_drag() -> bool:
	return _was_scrolled
...
func _start_content_drag() -> void:
	...
	is_in_deadzone = true
	_drag_started = true      # Запоминаем, что начали перетаскивание
	_was_scrolled = false      # Сбрасываем флаг прокрутки
	_focus_released = false    # Сбрасываем флаг снятия фокуса
...
func _end_content_drag() -> void:
	...
	_drag_started = false
	# Не сбрасываем _was_scrolled здесь, так как он нужен для определения,
	# была ли прокрутка во время этого нажатия
...
func _process_drag_motion(event) -> void:
	...
	# Сохраняем предыдущее накопленное движение
	var old_movement = Vector2(drag_temp_data[0], drag_temp_data[1]).length()
	
	# Обновляем накопленное движение
	if _container.should_scroll_horizontal():
		drag_temp_data[0] += event.relative.x
	if _container.should_scroll_vertical():
		drag_temp_data[1] += event.relative.y
	
	var new_movement = Vector2(drag_temp_data[0], drag_temp_data[1]).length()
	
	# Проверяем: если мы все еще в deadzone и движение превысило порог
	if new_movement >= scroll_zone and not _focus_released:
		is_in_deadzone = false
		_focus_released = true
		_was_scrolled = true   # Отмечаем, что была прокрутка
...
func _process_pan_gesture(event: InputEventPanGesture) -> void:
	...
	# Панорамирование тоже считается прокруткой
	_was_scrolled = true
